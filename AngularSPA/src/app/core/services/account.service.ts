...
export class AccountService {
  ...
  Login(login:Login):Observable<boolean>{
    return this.http.post('https://localhost:7079/api/Account/login', login).pipe(map((response:any) => {
      if (response){
  ...
  Register(registration:Register):Observable<boolean>{
    return this.http.post<boolean>('https://localhost:7079/api/Account/register', registration);
  }
